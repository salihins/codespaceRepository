...
function trimNumber(price) {
  const clearedPrice = price.trim().replace(/[,\s.\s_]/g, "");
...
function resolveThreeDigits(givenNumber) {
    let returnString = "";
...
export function count(price) {
    const stringToNumber = trimNumber(price);
    return convertToString(stringToNumber);
}
